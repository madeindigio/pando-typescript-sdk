...
import type { AgentEvent, AgentEventParams } from "../src/types.js";
...
describe("parseAgentEvent", () => {
  ...
  it("parses tool_call events without optional fields", () => {
    const params: AgentEventParams = {
      type: "tool_call",
      sessionId: "sess-2",
      toolCall: {
  ...
  it("parses response events", () => {
    const params: AgentEventParams = {
      ...
      message: {
        role: "assistant",
        content: "The refactoring is complete.",
      },
    };
    const event = parseAgentEvent(params);
    expect(event).toEqual({
  ...
  it("parses response events without message (defaults)", () => {
    const params: AgentEventParams = {
      type: "response",
      sessionId: "sess-4",
    };
  ...
  it("returns false for non-terminal events", () => {
    const nonTerminal: AgentEvent[] = [
      ...
      },
      {
        type: "tool_result",
        sessionId: "s",
        toolResult: { toolCallId: "1", name: "bash", content: "", isError: false },
      },
